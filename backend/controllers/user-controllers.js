...
const getUserItems = async (req, res, next) => {
  ...
  if (existingUser) {
    res.status(200).json({ username: username, items: existingUser.items, wishlist: existingUser.wishlist });
  } else {
...
const addwishlist = async (req, res, next) => {
  ...
  res.status(201).json({username: username,items: existingUser.items, wishlist: existingUser.wishlist});
};
const removewishlist = async(req, res,next) => {
  ...
  try{ 
      ...
      return next(new HttpError("Item could not be removed from wishlist!!",500));
  }
  res.status(200).json({username: username,items: existingUser.items, wishlist: existingUser.wishlist});
}
