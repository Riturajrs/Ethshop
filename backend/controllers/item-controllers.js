...
conn.once("open", () => {
  ...
  console.log("Connection made for images");
});
...
const userItems = async (req, res, next) => {
  ...
  let resultArray;
  try {
    resultArray = AllItems.filter((item) =>
      existingUser.items.includes(item._id),
    );
  } catch (err) {
    new HttpError("Failed to fetch data from items", 500);
  }
...
const createItem = async (req, res, next) => {
  ...
  const { title, Metamask_add, description, creator, lPrice, hPrice } =
    req.body;
